...
if(Valid==1 && plateau_modification_changer_orientation_piece_etre_possible(plateau,x0,y0,orientation)==1)
 {
    plateau_modification_changer_orientation_piece(jeu->plateau,x,y,orientation);
    coup_jeu coup;
    coup_jeu_initialiser(&coup);
    return coup;
    printf("Le coup est valide"/n);
}
else{printf("Le coup est impossible"/n);}
}
